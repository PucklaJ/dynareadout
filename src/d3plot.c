...
#define PGNIND_PUSH()                                                          \
  if (*num_part_node_indices == 0) {                                           \
    *num_part_node_indices = 1;                                                \
    ...
    int found;                                                                 \
    const size_t insert_index = d3_word_binary_search_insert(                  \
        part_node_indices, 0, *num_part_node_indices - 1, node_index, &found); \
    if (!found) {                                                              \
...
#define PGNIND_ADD_ELEMENT(el_ids, el_cons, num_els, ids_func, cons_func,      \
                           con_type)                                           \
  if (part->num_els != 0) {                                                    \
    if (!PGNI_LOAD(el_ids, num_els, ids_func)) {                               \
      /* Ignore these elements*/                                               \
...
d3_word *d3plot_part_get_node_indices(d3plot_file *plot_file,
  ...
  *num_part_node_indices = 0;
  ...
  /* Add solids*/
  PGNIND_ADD_ELEMENT(solid_ids, solid_cons, num_solids,
                     d3plot_read_solid_element_ids, d3plot_read_solid_elements,
                     d3plot_solid_con);
  /* Add beams*/
  PGNIND_ADD_ELEMENT(beam_ids, beam_cons, num_beams,
                     d3plot_read_beam_element_ids, d3plot_read_beam_elements,
                     d3plot_beam_con);
  /* Add shells*/
  PGNIND_ADD_ELEMENT(shell_ids, shell_cons, num_shells,
                     d3plot_read_shell_element_ids, d3plot_read_shell_elements,
                     d3plot_shell_con);
  /* Add thick shells*/
  PGNIND_ADD_ELEMENT(thick_shell_ids, thick_shell_cons, num_thick_shells,
                     d3plot_read_thick_shell_element_ids,
                     d3plot_read_thick_shell_elements, d3plot_thick_shell_con);
